...
import pytest
from karabinerpyx import LayerStackBuilder, Rule
...
class TestComplexScenarios:
    ...
    def test_stacked_layer_activation(self):
        ...
        activation_rule = next(r for r in rules if "stacked activation" in r.description)
    ...
    def test_nested_layer_conditions(self):
        ...
        rules = layer.build_rules()
        mapping_rule = next(r for r in rules if "hyper: j → down_arrow" in r.description)
        ...
        app_cond = next(c for c in manip["conditions"] if c["type"] == "frontmost_application_if")
